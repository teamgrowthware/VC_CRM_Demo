...
import { ChevronLeft, ChevronRight, Loader2, Plus, Trash2, X, Settings2 } from 'lucide-react';
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <form onSubmit={handleAddHoliday} className="bg-zinc-800/30 p-4 rounded-lg border border-zinc-700/50 mb-6 flex flex-col gap-3">
                ...
                <div>
                  ...
                  <input required type="text" value={newHoliday.name} onChange={e => setNewHoliday({...newHoliday, name: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500" placeholder="e.g. Diwali" />
                </div>
                  ...
                  holidaysList.map(h => (
                    <div key={h.id} className="flex justify-between items-center bg-zinc-800/50 border border-zinc-700/50 p-3 rounded-lg group">
                      <div>
                        ...
                        <p className="text-xs text-zinc-500">{formatDate(h.date)} • {h.type}</p>
                      </div>
